 ...
 *
 */
define([ "jquery", "directives/ngGestures"], function ( $, ngGenstures) {

    angular.module('widget.scrollbar', ["ng-sobey-gestures"])
    ...
    .directive('ngScrollbar', [
        function () {
            return {
                ...
                template: '<div hm-pan="scrollPan($event)" style="position:relative;width:100%;height:100%; overflow:hidden;">\
                ...
                compile: function (element) {
                    ...
                    return function (scope, element, attrs, ctrl) {
                        ...
                        scope.scrollPan = function (ev) {
                            ctrl.scrollY(ev.deltaY);
                        };
                    };
                }
            };
    ...
    .directive('ngScrollbarY', ['$timeout', '$animate', function ($timeout, $animate) {
        return {
            restrict: 'AE',
            require: '^ngScrollbar',
            replace: true,
            ...
            compile: function () {
                return function (scope, element, attrs, ctrl) {
                    ...
                    var scroll = function (distance) {
                        if (distance.toString().indexOf('.')>0) {
                    ...
                    containerElement.on('mousewheel touchmove', function (event) {
                        if (!isOverflow()) {
                            return;
                        }
                        ...
                        scroll(parseFloat((-event.wheelDeltaY || -event.wheelDelta)).toFixed(1));
                    });
                    ...
                    docEl.on('mousemove', function (event) {
                        if (scrollbarMousedown) {
                            event.preventDefault();
                            var dis = (event.screenY - axisY) * getContentHeight() / getContainerHeight();
                            scroll(parseFloat(dis).toFixed(5));
                            axisY = event.screenY;
                        }
